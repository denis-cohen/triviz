...
#' @param type Specified whether uncertainty estimates are analytically or numerically
#' derived (evaluated only when \code{estimates} is a user-supplied object)
...
plot_point_estimates <- function(estimates,
  ...
  if (inherits(estimates, "triviz_estimates")) {
    ...
    p_val_type <- ifelse(type == "analytical",
                         "p-value",
                         "Pr(k+1 > k)")
